...
(() => {
  ...
  const hover = (e) => {
    ...
    if (d) {
      ...
      if (d.leader_move_bps != null) lines.push(`leader moved ${f(d.leader_move_bps)} bps since its reference ${esc(d.leader_ref)}${d.leader_jump_bps != null ? `, sharpest ${f(d.leader_jump_bps)} bps in one jump span` : ""}${d.leader_impulse_bps != null ? `, ${f(d.leader_impulse_bps)} bps off its short ema` : ""}`);
